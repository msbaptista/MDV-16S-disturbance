...
# The filtered data set explains slightly less variance (less 4%). This was considered a marginal difference. Both filtered and full data sets are giving the same results. We will use the filtered data set.
...
# Much more significant difference for Valley than Type, but still it is significant for both. PERMDISP not significant.
```
...
# Add a pseudo count and transform data with philr
dvf_dna_t <- transform_sample_counts(dvf_dna, function(x) x+1)

dvf_dna_t_philr <- philr(otu_table(dvf_dna_t),
                       phy_tree(dvf_dna_t),
                       part.weights='enorm.x.gm.counts',
                       ilr.weights='blw.sqrt')

# Calculate the distance with method "euclidean"
dvf_dna_t_dist <- vegdist(dvf_dna_t_philr, method = "euclidean")
...
# How much variance is explained by each axis - The first 2 axis explain  ca. 70 %
...
# Add a pseudo count
dvf_cdna_t <- transform_sample_counts(dvf_cdna, function(x) x+1)

# Transform with philr
dvf_cdna_t_philr <- philr(otu_table(dvf_cdna_t),
                       phy_tree(dvf_cdna_t),
                       part.weights='enorm.x.gm.counts',
                       ilr.weights='blw.sqrt')

# Calculate the distance with method "euclidean"
dvf_cdna_t_dist <- dist(dvf_cdna_t_philr, method = "euclidean")
...
# Select ASVs that are rare at T0, i.e. < 10 CSS reads and then > =10 CSS at T1, T2 or T3 


# Select only DNA samples because we don't have T0 for RNA
dvf2_css_d <- subset_samples(dvf2_css, Type == "DNA") %>%  
  prune_taxa(taxa_sums(.) > 0, .)

# Subset to T0
dvf2_css_d_t0 <- subset_samples(dvf2_css_d, Time == "T0") %>%
  prune_taxa(taxa_sums(.) > 0, .)

# Create a data frame
dvf2_css_d_t0_asv <- data.frame(otu_table(dvf2_css_d_t0))

# Select for Beacon
dvf2_css_d_t0_beacon <- dvf2_css_d_t0_asv %>%
  select(X00B) %>%
  filter(X00B < 10)

# Get the ASVs
beacon_d_asvs <- rownames(dvf2_css_d_t0_beacon) 

# Select for Miers
dvf2_css_d_t0_miers <- dvf2_css_d_t0_asv %>%
  select(X00M) %>%
  filter(X00M < 10)

# Get the ASVs
miers_d_asvs <- rownames(dvf2_css_d_t0_miers) 


# Create a phyloseq object with the selected ASVs 

beacon_dna <- dvf2_css_d %>%                    
  subset_samples(Valley == "Beacon") %>%
  prune_taxa(taxa_sums(.) > 0, .)

beacon_dna <- prune_taxa(beacon_d_asvs, beacon_dna) # 79 ASVs

miers_dna <- dvf2_css_d %>%                    
  subset_samples(Valley == "Miers") %>%
  prune_taxa(taxa_sums(.) > 0, .)

miers_dna <- prune_taxa(miers_d_asvs, miers_dna) # 104 ASVS

# Get data for plotting

# Create a data frame
beacon_dna_dt <- psmelt(beacon_dna)

# Plot

seed_beacon_dt <- beacon_dna_dt %>%
  filter(Time != "T0") %>% 
  filter(Abundance > 9)

seed_beacon_dt$Treatment <- factor(seed_beacon_dt$Treatment,
                                   levels=c("NaCl", "Cu", "OM", "HighN", "LowN", "Water"))

seed_beacon <- ggplot(seed_beacon_dt, aes(x = Abundance, after_stat(count), fill = Time)) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("#E64B35FF", "#4DBBD5FF", "#00A087FF")) +
  scale_x_log10(breaks = c(10, 100,  500)) +
...
# Create a data frame
miers_dna_dt <- psmelt(miers_dna)

# Plot

seed_miers_dt <- miers_dna_dt %>% 
  filter(Time != "T0") %>% 
  filter(Abundance > 9)

seed_miers_dt$Treatment <- factor(seed_miers_dt$Treatment,
                                   levels=c("NaCl", "Cu", "OM", "HighN", "LowN", "Water"))

seed_miers <- ggplot(seed_miers_dt, aes(x = Abundance, after_stat(count), fill = Time)) +
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("#E64B35FF", "#4DBBD5FF", "#00A087FF")) +
  scale_x_log10(breaks = c(10, 100, 500)) +
  scale_y_continuous(breaks = c(0, 20, 40, 60)) +
...
seed_beacon / seed_miers

#ggsave("Plots_Seed_density.png", dpi = 300, width = 8, height = 6)
```
...
seed_beacon_taxa <- seed_beacon_dt %>% 
  filter(Time != "T0") %>% 
  filter(Abundance > 9) %>% 
  ggplot(aes(Abundance, reorder(ASV, desc(Phylum)))) +
  geom_line(aes(colour = Phylum), size = 1, alpha = 0.6) +
  geom_point(aes(colour = Phylum), size = 1.7) +
  scale_x_log10() +
  scale_colour_manual(values = c("#F5CAC3", "#F6BD60","#84A59D",
                                 "#F28482", "#3D405B", "#E07A5F",
                                 "#81B29A", "#F2CC8f", "#227C9D",
                                 "#FE6D73")) +
...
# Use plotly to get an interactive plot and quickly inspect which ASVs are changing in each disturbance
ggplotly(seed_beacon_taxa)

seed_miers_taxa <- seed_miers_dt %>% 
  filter(Time != "T0") %>% 
  filter(Abundance > 9) %>% 
  ggplot(aes(Abundance, reorder(ASV, desc(Phylum)))) +
  geom_line(aes(colour = Phylum), size = 1, alpha = 0.6) +
  ...
  scale_colour_manual(values = c("#F5CAC3", "#F6BD60","#84A59D",
                                 "#F28482", "#3D405B", "#E07A5F", 
                                 "#81B29A", "#17C3B2","#F2CC8f", 
                                "#227C9D", "#FFCB77", "#FE6D73")) +
  scale_x_log10() +
  facet_grid(~Treatment,
             labeller = labeller(Treatment = disturb.label)) +
  theme_cowplot() +
...
ggplotly(seed_miers_taxa)

#seed_beacon_taxa / seed_miers_taxa
#ggsave("Plots_Seed_taxa.png", dpi = 300, width = 12, height = 14)
...
# How many ASVs do the different disturbances have in common? Use an upSet plot to inspect 

# Get Treatment as a variable
beacon_dna_up <- beacon_dna_dt %>% 
...
beacon_dna_up_nacl <- beacon_dna_up %>% 
  filter(NaCl > 9)
...
list_beacon_d <- list(
  Conductivity = beacon_dna_up_nacl$ASV,
  Copper = beacon_dna_up_cu$ASV,
  Glucose = beacon_dna_up_om$ASV,
  "High N" = beacon_dna_up_hn$ASV,
  "Low N" = beacon_dna_up_ln$ASV,
  Water = beacon_dna_up_nacl$ASV)

# Upset plot Beacon
up_beacon_d <- UpSetR::upset(fromList(list_beacon_d), 
...
# Get Treatment as a variable
miers_dna_up <- miers_dna_dt %>% 
  select(Abundance, Treatment, Time, ASV) %>%
  pivot_wider(names_from = Treatment, values_from = Abundance)

# Create data frames for each treatment  
miers_dna_up_nacl <- miers_dna_up %>% 
  filter(NaCl > 9)
...
# Name the disturbances
list_miers_d <- list(
  Conductivity = miers_dna_up_nacl$ASV,
...
# Upset plot Miers DNA
up_miers_d <- upset(fromList(list_miers_d), 
                    sets = c("Water", "Low N", "High N", "Glucose", "Copper", "Conductivity"),
                    order.by = "freq", keep.order = TRUE)
up_miers_d
...
up_beacon_d_plot <- cowplot::plot_grid(
  NULL, up_beacon_d$Main_bar, up_beacon_d$Sizes, up_beacon_d$Matrix,
  nrow = 2, align = "hv", rel_heights = c(3,1), rel_widths = c(2,3), labels = "a")

up_miers_d_plot <- cowplot::plot_grid(
  NULL, up_miers_d$Main_bar, up_miers_d$Sizes, up_miers_d$Matrix,
...
# High N has the highest number of unique ASVs in both valleys, but they are not the same
list_high_n <- list(
  "Beacon" = beacon_dna_up_hn$ASV,
  "Miers" = miers_dna_up_hn$ASV)

upset(fromList(list_high_n),
                    order.by = "freq", keep.order = TRUE)
...
# Add a pseudo count
dvf2_f0_m_t <- transform_sample_counts(dvf2_f0_m, function(x) x+1)

# Transform with philr
dvf2_f0_m_t_philr <- philr(otu_table(dvf2_f0_m_t),
                           phy_tree(dvf2_f0_m_t),
                           part.weights='enorm.x.gm.counts',
                           ilr.weights='blw.sqrt')

# Calculate the distance with method "euclidean"
dvf2_f0_m_t_dist <- dist(dvf2_f0_m_t_philr, method = "euclidean")
...
# looking at the scree plot we could have gone up to 10 axes - I tried but there was no further information to be gained

# Create a data frame with Axis information 
distance_df <- data.frame(dvf2_f0_m_t_pcoa$vectors) %>%
  select(Axis.1, Axis.2, Axis.3, Axis.4, Axis.5, Axis.6)
...
ann_text <- data.frame(Time = "T3", dist = 1, lab = "Text",
...
```{r DNA - RNA difference ASVs}

# Transform phyloseq object in a data frame
miers_ratio_melt <- dvf2_f0_m %>%
  psmelt() %>%
  select(Abundance, Treatment, Time, Days, Type, Phylum, ASV, Genus_ASV)

# Long-to-wide to get the variables RNA and DNA
miers_ratio <- miers_ratio_melt %>% 
  pivot_wider(names_from = Type, values_from = Abundance) %>%
  filter(RNA > 0)

# How many phantom taxa? 
# RNA reads > 1 & DNA reads = 0 -> Phantom taxa

miers_ratio_phantom <- miers_ratio %>% 
  filter(DNA == 0) # 1232 obs. from 2754 = 45 % ASVs are phantom taxa

# Add 1 to every DNA entry of 0
miers_ratio$DNA[miers_ratio$DNA == 0] <- 1

# Calculate log2 RNA:DNA
miers_ratio <-miers_ratio %>%
  mutate(logratio = log2(RNA /DNA)) %>%
  filter(abs(logratio) >= 2) # Define a threshold of 2 for considering activity

# Plot

miers_ratio$Treatment <- factor(miers_ratio$Treatment,
                                       levels=c("NaCl", "Cu", "OM", 
                                                "HighN", "LowN", "Water"))

plot_diff_rna_phantom <- ggplot(miers_ratio, 
                                aes(x = logratio, y = reorder(ASV, desc(Phylum)),
                                    colour = Phylum, yend = ASV, xend = 0)) +
  ...
  labs(x = "Log2 RNA:DNA", y = "", 
       tag = "b",
       colour = "")

# ggplotly(plot_diff_rna_phantom)


# For plotting remove phyla with 3 or less ASVs

remov_phyla <- c("BJ-169", "Chlamydiae", "Chlorobi", "Elusimicrobia",
                 "Fibrobacteres", "Firmicutes", "Parcubacteria", "Saccharibacteria")

miers_ratio <- miers_ratio %>%
  filter(Phylum != "" ) %>% 
  filter(!Phylum %in% remov_phyla)
...
plot_diff_rna_phantom <- ggplot(miers_ratio, 
  ...
  geom_segment() +
  geom_vline(xintercept = 0) +
  scale_colour_manual(values = c("#F5CAC3", "#84A59D", "#F6BD60",
                                 "#E07A5F", "#F28482", "#3D405B", 
                                 "#81B29A", "#E64B35FF", "#17C3B2",
                                 "#F2CC8f", "#227C9D", "#FFCB77", 
                                 "#4DBBD5FF", "#FE6D73")) +
  facet_grid(~Treatment,
             labeller = labeller(Treatment = disturb.label),
             scales = "free_y") +
...
#plot_diff_a / plot_diff_rna_phantom +
#   plot_layout(heights = c(1, 2))

#ggsave("DNA-RNA_difference.png", dpi = 300, width = 14, height = 12)

miers_ratio %>%
  filter(Treatment == "NaCl") %>% # 184
  count(ASV)

miers_ratio %>%
  filter(Treatment == "Cu") %>% # 187
  count(ASV)

miers_ratio %>%
  filter(Treatment == "OM") %>% # 122
  count(ASV)

miers_ratio %>%
  filter(Treatment == "HighN") %>% # 167
  count(ASV)

miers_ratio %>%
  filter(Treatment == "LowN") %>% # 402
  count(ASV)

miers_ratio %>%
  filter(Treatment == "Water") %>% # 208
  count(ASV)
